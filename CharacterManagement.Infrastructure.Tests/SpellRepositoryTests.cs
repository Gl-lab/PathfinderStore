...
public sealed class SpellRepositoryTests
{
    ...
    public void GetAll_ReturnsCompleteUniquePlayerCoreLevelOneCatalog()
    {
        ...
        Assert.Equal( 128, spells.Count );
        Assert.Equal( 128, spells.Select( spell => spell.Id ).Distinct( StringComparer.Ordinal ).Count() );
        Assert.Equal( 26, spells.Count( spell => spell.Kind == SpellKind.Cantrip ) );
        Assert.Equal( 58, spells.Count( spell => spell.Kind == SpellKind.Spell ) );
        Assert.Equal( 44, spells.Count( spell => spell.Kind == SpellKind.Focus ) );
