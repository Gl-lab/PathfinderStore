...
export default class NucleusService {
  ...
  public put<T>(url: string, data: any | string): Promise<IRestResponseDto<T>> {
    return NucleusService.request<T>('PUT', url, data);
  }

  public post<T>(url: string, data: any | string): Promise<IRestResponseDto<T>> {
    return NucleusService.request<T>('POST', url, data);
  }

  public postOrPut<T>(url: string, data: any | string, id: string | undefined): Promise<IRestResponseDto<T>> {
    if (id && id !== Guid.empty) {
      return NucleusService.request<T>('PUT', url, data);
    }
