    ...
    : IDesignTimeDbContextFactory<InventoryDbContext>
{
    public InventoryDbContext CreateDbContext( string[] args )
    {
        ...
        IConfigurationRoot configuration = new ConfigurationBuilder()
            ...
            .Build();
        string connectionString = configuration[ "DB:Inventory" ]
            ?? configuration[ "DB:CharacterManagement" ]
            ?? configuration[ "DB_CONNECTION" ]
            ?? throw new InvalidOperationException(
                $"Connection string 'DB:Inventory' was not found in '{configurationBasePath}'." );
        DbContextOptionsBuilder<InventoryDbContext> builder =
    ...
    private static string ResolveConfigurationBasePath()
    {
        ...
        throw new InvalidOperationException(
            "Could not locate the Pathfinder.Web configuration directory for design-time InventoryDbContext creation." );
    }
