# AGENTS.md — Инструкции для AI-агентов

Этот файл является единственным источником инструкций для всех AI-агентов (Claude, Cursor, Copilot и др.).

---

## Обязательное применение

Перед началом **каждой** сессии агент обязан:
1. Прочитать `AGENTS.md` полностью.
2. Прочитать обязательные файлы из раздела `MemoryBank`.
3. Только после этого приступать к анализу, правкам кода и ответам пользователю.

Игнорирование этих пунктов считается нарушением регламента проекта.

---

## MemoryBank

Папка `MemoryBank/` содержит актуальный контекст проекта. Файлы поддерживаются в актуальном состоянии по мере работы над проектом.

### Читать обязательно в начале каждой сессии

| Файл | Содержимое |
|------|-----------|
| `MemoryBank/00_project/project_overview.md` | Обзор проекта: стек, структура, конфигурация dev-среды |
| `MemoryBank/10_workflow/sandbox.md` | Практические ограничения sandbox и признаки, когда нужна escalation |
| `MemoryBank/10_workflow/vikunja.md` | Подробные правила работы с задачами через MCP Vikunja |

### Читать обязательно только при разработке

| Файл | Содержимое |
|------|-----------|
| `MemoryBank/10_workflow/feedback_csharp_style.md` | Правила стиля C# кода |
| `MemoryBank/10_workflow/ef.md` | Проверенный workflow для EF migrations и связанные known issues |

### Читать обязательно при исследованиях

| Файл | Содержимое |
|------|-----------|
| `MemoryBank/10_workflow/aon_elasticsearch_usage.md` | Как использовать AoN Elasticsearch в исследовательских сессиях |

### Читать по необходимости

| Файл | Содержимое |
|------|-----------|
| `MemoryBank/10_workflow/avatar_assets.md` | Пошаговый workflow генерации, проверки, конвертации и подключения аватаров |
| `MemoryBank/00_project/project_pathfinder_character_domain.md` | Детальный анализ CharacterManagement: что реализовано, что нет |
| `MemoryBank/30_task_notes/mvp_character_creation_backend.md` | Статус backend MVP создания персонажа |
| `MemoryBank/30_task_notes/mvp_character_creation_frontend.md` | Статус frontend MVP создания персонажа |
| `MemoryBank/30_task_notes/task_14_integration_test_context.md` | Контекст по интеграционным тестам CharacterManagement |
| `MemoryBank/20_domain/character_creation/` | Нормативные доменные правила создания персонажа |
| `MemoryBank/90_research/` | Временные исследовательские заметки и выгрузки |

---

## Контекст проекта

Актуальный стек, структура solution, dev setup, API и статус модулей описаны только в [`MemoryBank/00_project/project_overview.md`](MemoryBank/00_project/project_overview.md).

Не дублировать в этом файле изменчивые сведения о реализации и статусе задач: это приводит к рассинхронизации правил для агентов и проектной документации.

---

## Serena

Перед первым анализом или изменением кода в каждой сессии:

1. Определить корень текущего Git-репозитория командой `git rev-parse --show-toplevel`.
2. Активировать полученный корень репозитория как текущий проект через Serena.
3. Прочитать начальные инструкции Serena и использовать её символьные инструменты для навигации и семантического анализа кода.
4. Не использовать захардкоженные пути к локальному каталогу проекта.
5. Если Serena недоступна или не смогла активировать проект, явно сообщить об этом перед продолжением анализа или изменения кода.

---

## Задачи — MCP Vikunja

Для ведения задач используется **MCP Vikunja** (`@0xk3vin/vikunja-mcp`).

Подробные правила, конфигурация, known issues и контракт инструментов вынесены в `MemoryBank/10_workflow/vikunja.md`.

Короткие обязательные правила:
- перед созданием новой задачи всегда сначала проверять существующие задачи проекта;
- каждая задача должна явно описывать проблему, которую она решает, и проверяемый признак её устранения;
- при работе с кириллицей в задачах использовать только UTF-8 и после записи перепроверять отображение;
- если MCP-инструменты начинают отвергать числа или булевы значения, свериться с `MemoryBank/10_workflow/vikunja.md` и перепроверить локальный патч.

---

## Правила написания C# кода

Строго соблюдать при написании **любого** C# кода в проекте:

1. **Нет `var`** — всегда явный тип
2. **`_camelCase`** для всех полей (приватных, публичных, статических)
3. **`PascalCase`** для констант, методов, свойств, типов
4. **Скобки всегда** обязательны, открывающая на новой строке
5. **Пробелы внутри скобок:** `Foo( arg )`, `if ( x )`, `arr[ i ]`, `( int )obj`
6. **Цепочки методов** — каждый вызов на новой строке
7. **Конструкторы** — никогда expression-bodied
8. **Аксессоры** — всегда expression-bodied
9. **Явные скобки** в бинарных/реляционных выражениях
10. **Без `this.`**
11. **Предпочитать объектные/коллекционные инициализаторы**
12. **Модификаторы доступа** — всегда указывать
13. **`readonly`** — везде где возможно
14. **`sealed`** — если класс не предназначен для наследования
15. Все работы с миграциями выполнять через консоль dotnet ef

---

## Правила документации

1. В проектной документации использовать только относительные пути и ссылки внутри репозитория
2. Не использовать абсолютные пути вида `D:\...` или `C:\...` для файлов проекта
3. Если нужно сослаться на внешний путь вне репозитория, использовать переменные окружения или текстовое описание без абсолютной ссылки
