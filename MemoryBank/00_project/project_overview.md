...
- `Commerce.Domain` — campaign-scoped поселения, магазины, ручные предложения, версионируемые правила и воспроизводимые запуски пополнения, ценовые политики, кошельки, денежный журнал, резервы и сделки.
- `Commerce.Application` — административные и пользовательские торговые сценарии, генерация preview по seed и версии правила, GM review, DTO и межконтекстные порты.
...
Отвечает за stable item key, последовательность неизменяемых ревизий, global/campaign scope, типизированные attack/armor/shield/equipment/consumption/charge/durability components и неизменяемые configurations из ревизии, размера, материала и постоянных улучшений. Ревизии проходят lifecycle `Draft -> Published -> Retired`; глобальными описаниями управляет системный администратор, campaign-описаниями — активный ведущий точной кампании. Реализованы campaign-scoped уникальные configurations, полное серверное и видимое наблюдателю представления, hidden permanent upgrades и знания персонажа или партии.
...
`Commerce` является целевой реализацией магазинов и ручной торговли. Он хранит поселения, магазины, конечный ассортимент, неизменяемые версии правил пополнения, воспроизводимые preview-запуски, ценовые политики, кошельки, резервы и снимки завершённых сделок. Подтверждение запуска идемпотентно публикует предложения и уникальные экземпляры, отклонение не меняет ассортимент. Физические экземпляры остаются в `Inventory`; описание, базовая цена, категория и редкость читаются из `ItemCatalog`; campaign access разрешается через `CampaignManagement`.
...
- Backend MVP создания персонажа реализован:
  ...
  - `POST /api/item-catalog-admin/definitions/{id}/revisions` (черновик новой ревизии существующего вида);
  ...
  - `GET /api/item-catalog-admin/definitions` (административный список видов и ревизий: scope/campaignId/status/search/paging);
  - `GET /api/item-catalog-admin/capabilities` (права текущего пользователя на глобальный и кампанийный каталог);
  - `POST /api/item-catalog-admin/campaigns/{campaignId}/configurations` (идемпотентная стандартная конфигурация кампании по `ConfigurationKey`);
...
- [`../30_task_notes/item_catalog_admin_spec.md`](../30_task_notes/item_catalog_admin_spec.md)
