...
| Legacy `Store.*` | Удалён из solution; оставшиеся controller stubs не являются целевой моделью |
...
- gift, exchange, party storage и forced GM move опубликованы как runtime-команды;
- `Commerce` владеет campaign-scoped поселениями, магазинами, ручными предложениями, кошельками, денежным журналом, резервами и зафиксированными сделками; передача физических экземпляров выполняется через `Inventory`;
- автоматическое пополнение ассортимента и расширенный lifecycle предметов реализованы; UI управления торговлей остаётся следующим этапом;
...
В текущем backend кошелёк принадлежит персонажу внутри точной кампании (`CampaignId` + `CharacterId`). Поддержка кошельков учётной записи или отдельных экономических сущностей, если она понадобится, требует нового явного решения и не меняет текущий контракт.
...
| Идентичность | Ревизия → конфигурация → экземпляр реализованы | Достигнуто для базовой модели |
| Поведение | Типизированные attack/armor/shield/equipment/consumption/charge/durability components | Расширяемый состав компонентов — базовая цель достигнута |
| Состояние | Количество, расположение, версия, заряды, расходование, прочность и прикрепляемые руны реализованы | Базовая runtime lifecycle boundary достигнута |
| Владение | Campaign-scoped контейнеры и журнал операций реализованы | Достигнуто для backend command boundary |
| Кампании и партии | Кампания, членство, роли, одна активная партия и назначения реализованы | Read projections/UI и расширение до нескольких партий |
| Авторизация ведущего и игрока | Campaign-scoped роли и policies реализованы | Достигнуто для текущих backend-сценариев |
...
| Изоляция игрового мира | `CampaignId` применяется к runtime-экземплярам, контейнерам, магазинам и сделкам | Достигнуто для текущих bounded contexts |
...
| Магазины | Поселения, магазины, ручные предложения и ценовые политики реализованы | Генерация/пополнение ассортимента, read projections и UI |
| Сделки | Кошельки, резервы, денежный журнал, покупка и продажа реализованы | Достигнуто для ручного backend flow; остаются projections/UI |
...
| Этап 5 | Priority 13: магазины и ручная торговля — завершён 23 июля 2026 года |
| Пользовательский UI MVP-1 | Priority 15: read-проекции Inventory/Commerce, страница кампании, инвентарь, дарение и партийное хранилище |
| Этап 6 | Priority 14: инструменты ведущего — завершён 26 июля 2026 года; Priority 16: генерация ассортимента — завершён 27 июля 2026 года |
| Этап 7 | Priority 17: расширенный жизненный цикл предметов |
| Пользовательский UI MVP-2/MVP-3 | Priority 18: магазин игрока, обмен и панель торговли ведущего с блокирующими read-проекциями |
...
**Статус:** завершён; итоговая проверка зафиксирована в [Priority 13 Final Cross-Review](../../30_task_notes/priority_13_final_review.md).
...
**Статус этапа:** Priority 14 и Priority 16 завершены; итоговые проверки зафиксированы в [Priority 14 Final Cross-Review](../../30_task_notes/priority_14_final_review.md) и [Priority 16 Final Cross-Review](../../30_task_notes/priority_16_final_review.md). Между ними завершён пользовательский UI MVP-1 из Priority 15.
...
- заряды и восстановление — реализовано;
- расходование и боеприпасы — реализовано;
- прочность, поломка, уничтожение и ремонт — реализовано;
- руны и перенос улучшений — реализовано;
- trusted integration port для будущих игровых действий — реализовано без encounter engine.
...
25. Priority 8–17 завершены; оставшийся торговый UI следует в Priority 18.
...
1. Нужны ли кроме реализованного кошелька персонажа кошельки учётной записи или отдельных экономических сущностей.
...
8. Нужно ли удалить оставшиеся legacy shop-controller stubs после удаления проектов `Store.*`.
