...
- каждая партия получает пустой корневой storage contract с политикой `Unconfigured`; модель предметов и перемещений остаётся ответственностью будущего `Inventory`;
...
- пока поддерживается только одна активная партия на кампанию, а политика доступа к общему хранилищу ещё не настроена;
- корневое хранилище партии не содержит runtime-предметов, перемещений и журнала операций;
...
25. Priority 8 и Priority 9 завершены; каталог, экземпляры, обмен и торговля следуют по Priority 10–16.
