...
5. `CharacterManagement` получает разрешённое описание конкретного предмета через собственный прикладной контракт;
...
- `CharacterManagement` владеет минимальным каталогом стартового снаряжения, классовыми наборами и состоянием экипировки черновика и завершённого персонажа;
- персонаж хранит стабильный идентификатор вида предмета, приобретённое количество и экипированное количество;
- стоимость, применимое владение и нагрузка вычисляются сервером;
- текущий `Store` отключён в web composition root;
- `TeamsController` содержит только пустые HTTP-заглушки и не задаёт модель кампании, партии или полномочий ведущего;
- `Secure` содержит глобальные роли и permissions, но не поддерживает назначение ведущего и игрока в области отдельной кампании;
- изменение инвентаря после создания, магазины, сделки, заряды, расходование, руны и повреждение предметов отсутствуют;
...
- нет постоянной личности физического предмета;
- нельзя различить два одинаковых меча с разной историей или состоянием;
- нет владельца runtime-инвентаря;
- описание предмета не версионируется;
- нет журнала перемещений и защиты от повторных операций;
- отсутствует модель знаний о скрытых свойствах;
- нет кампаний, членства, партий, привязки персонажа к игроку в партии и общего хранилища;
- глобальная роль пользователя не может корректно выразить участие одного человека в разных кампаниях с разными полномочиями;
...
6. Для партии создаётся общее хранилище по выбранной политике доступа.
...
1. Игровой процесс получает полное разрешённое описание экземпляра.
...
### 6.2. Чтение разрешённого предмета

`CharacterManagement` не должен напрямую знать хранилище каталога или самостоятельно объединять основу, руны и состояние экземпляра. Он объявляет собственный прикладной порт чтения разрешённого предмета.

```mermaid
flowchart LR
    CM["CharacterManagement"]
    Reader["Чтение разрешённого предмета"]
    Inventory["Инвентарь<br/>экземпляр и состояние"]
    Catalog["Каталог предметов<br/>описание и конфигурация"]

    CM -->|"Разрешённое описание"| Reader
    Reader --> Inventory
    Reader --> Catalog
```
...
- `IResolvedItemReader` — полное разрешённое описание для доверенных серверных правил;
...
- доступные активации;
- текущее состояние зарядов, прочности и поломки;
...
- вычисленные уровень, цену и нагрузку, если они зависят от состава.
...
| Щит | Бонус поднятого щита, твёрдость, прочность и порог поломки |
...
| Прочность | Твёрдость, максимальная прочность, поломка, уничтожение и ремонт |
...
| Текущая прочность и поломка | Состояние экземпляра |
| Количество в стопке | Экземпляр |
| Владение и расположение | Инвентарь |
| Итоговый бросок атаки и урона | Вычисление игрового процесса |

В Pathfinder 2e размер изготовленного оружия не масштабирует базовый кубик урона по старой таблице Pathfinder 1e. Он влияет на совместимость, цену и нагрузку по действующим правилам. Если проект когда-либо поддержит другой набор правил, масштабирование оформляется отдельной политикой набора правил, а не встраивается в общий экземпляр.

### 7.5. Изменяемое состояние экземпляра

Состояние создаётся только для присутствующих компонентов:

- `ItemDurabilityState` — текущая прочность, поломка и уничтожение;
...
| Область | Сейчас | Цель |
|---|---|---|
| Каталог | Минимальный статический каталог в `CharacterManagement` | Версионируемый `ItemCatalog` |
...
| Обмен игроков | Отсутствует | Подарок, атомарный обмен и общее хранилище партии |
| Изоляция игрового мира | Отсутствует | `CampaignId` у runtime-экземпляров, магазинов и мировых контейнеров |
| Боевая интеграция | Прямое разрешение минимального каталога | Прикладной порт разрешённого экземпляра |
...
| Администрирование | Отсутствует | Черновики, публикация, создание экземпляров и аудит |
...
| Этап 1 | Priority 9: кампании и партии |
| Этап 2 | Priority 10: версионируемый каталог предметов v2 |
| Этапы 3–4 | Priority 11: экземпляры, инвентарь и миграция; Priority 12: обмен партии |
...
| `CharacterManagement` начинает знать внутренности каталога | Сильная связанность | Собственный прикладной порт и собственные типы разрешённого описания |
...
8. Чтение разрешённого предмета и изменение его состояния разделены.
...
12. Текущее стартовое снаряжение остаётся действующим переходным состоянием до отдельной миграции.
...
25. Сначала реализуется Priority 8 с абстрактной границей чтения экипировки; кампании, каталог, экземпляры, обмен и торговля следуют после него по Priority 9–16.

---

## 13. Открытые решения

1. Русские термины для `Bulk`, item definition, variant, instance, loadout, effect и других понятий.
2. Владелец денег: персонаж, учётная запись или отдельная экономическая сущность.
3. Точный список компонентов первого релиза и формат параметров редких эффектов.
4. Является ли размер частью переиспользуемой конфигурации всегда или допускается неизменяемым параметром экземпляра для простых предметов.
5. Нужны ли мета-предметы на уровне учётной записи; обычные игровые экземпляры остаются внутри кампании.
6. Модель длительных резервов и компенсаций при будущем физическом разделении модулей.
7. Правила идентификации, ошибочной идентификации и автоматического раскрытия проклятий.
8. Объём прочности: только щиты в первом срезе или все damageable objects.
9. Стратегия удаления, архивации или миграционного использования legacy `Store.*`.
10. Нужны ли помощник ведущего и пользовательские наборы полномочий в первом релизе.
11. Может ли один персонаж одновременно состоять более чем в одной партии или кампании.
12. Требуется ли подтверждение получателя для всех подарков или только для ограниченных и уникальных предметов.
13. Исходная политика общего хранилища: свободное получение, подтверждение ведущего или разрешения по участникам.
14. Когда знание персонажа автоматически становится общим знанием партии.
15. Нужны ли общий кошелёк партии и коллективная оплата покупок в первом торговом срезе.
16. Как обрабатывать предметы персонажа при выходе игрока, персонажа или ведущего из кампании.
17. Допускается ли контролируемый импорт персонажа и его предметов между кампаниями.
