...
`CharacterBuilder` содержит будущие шаги:
...
Для текущего MVP это не считается незавершённой реализацией. Эти методы обозначают будущие этапы полного character creation.

Когда появятся задачи по `Background` и `Class`, их нужно реализовывать как отдельные catalog/rules flows, а не как свободные поля на персонаже.

## Legacy Item Endpoints

В `CharacterController` остаются legacy item endpoints:

- `GET /api/character/items`;
- `DELETE /api/character/items/drop`.

Их не считать частью Character Creation MVP. Если появится задача по inventory/store, нужно отдельно решить, остаются ли они в `CharacterManagement` или переезжают в более подходящий bounded context.
