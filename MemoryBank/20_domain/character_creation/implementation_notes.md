...
Активный create pipeline `CharacterBuilder` использует catalog-backed `SetBackground`, `SetClass` и `SetStartingEquipment`. Starting equipment больше не является placeholder: стоимость, equipped state, proficiency applicability, общая Масса и нагрузка вычисляются сервером.
