...
| `Ancestry`, background skill и class feats 1 уровня | Есть нормализованный Player Core catalog и общий набор полученных способностей персонажа | [../../30_task_notes/feat_catalog_player_core_implementation.md](../../30_task_notes/feat_catalog_player_core_implementation.md) |
...
Baseline-каталог Core Rulebook реализован в C# и участвует в character creation. Skill и Lore grants применяются; fixed skill feat добавляется в общий набор полученных способностей, а выбор `Martial Disciple` валидируется и сохраняется.
...
Реализован единый tradition-aware Player Core catalog и стартовые наборы заклинаний Cleric, Bard, Druid, Witch и Wizard: repertoire, preparation, spellbook, granted spells, class-specific slots и focus packages. Runtime casting, расходование ресурсов, rest lifecycle и исполнение effects остаются отдельной подсистемой.
...
Heritage и ancestry feats 1 уровня каталогизированы и входят в общий набор полученных способностей. Фиксированные training effects Lore feats и `Prairie Rider` применяются доменом; остальные эффекты остаются typed dependencies. Детальная матрица находится в [ancestry_choices_player_core.md](ancestry_choices_player_core.md).
...
3. Priority 11–16 из roadmap завершены: реализованы экземпляры и инвентарь, обмен, магазины, инструменты ведущего и воспроизводимая генерация ассортимента. Следующие предметные этапы — расширенный жизненный цикл в Priority 17 и оставшийся торговый UI в Priority 18.
