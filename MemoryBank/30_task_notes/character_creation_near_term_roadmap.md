...
- расчётные сервисы не зависят напрямую от `EquipmentRepository`, persistence-моделей или implementation types `ItemCatalog`/будущего `Inventory`;
...
**Статус:** завершён 21 июля 2026 года; результаты и общий quality gate зафиксированы в [итоговом cross-review](priority_9_final_review.md).
...
**Исходная проблема.** `TeamsController` являлся пустой legacy-заглушкой, глобальные роли `Secure` не выражали роль пользователя в конкретной игре, а будущие предметы и магазины не имели границы изоляции между играми.
...
**Статус:** завершён 22 июля 2026 года; результаты и общий quality gate зафиксированы в [итоговом cross-review](priority_10_final_review.md).
...
**Статус приоритета 11:** завершён 22 июля 2026 года; выполнен [итоговый cross-review](priority_11_final_review.md). Все семь slices реализованы отдельными коммитами. Завершённые персонажи, однозначно назначенные кампании, мигрируют в campaign-scoped runtime Inventory; боевая карточка читает точную catalog revision экземпляра. Черновики остаются в starting-equipment flow, а неоднозначно или ещё не назначенные персонажи сохраняют безопасный переходный fallback до появления явной кампании.
...
**Статус приоритета 12:** завершён 22 июля 2026 года; выполнен [итоговый cross-review](priority_12_final_review.md). Все пять slices реализованы отдельными коммитами. Итоговое ревью дополнительно закрепило явный запрет передачи на экземпляре: обычные gift, exchange и party storage операции его соблюдают, а исключение ведущего остаётся отдельной аудируемой командой с обязательной причиной.
...
**Статус приоритета 13:** завершён 23 июля 2026 года; выполнен [итоговый cross-review](priority_13_final_review.md). Все шесть slices реализованы отдельными коммитами. Итоговое ревью дополнительно защитило денежный журнал от повторного использования `OperationId`, добавило освобождение истёкшего резерва и отделило пользовательские торговые команды от административного API.
...
**Статус приоритета 14:** завершён 26 июля 2026 года; выполнен [итоговый cross-review](priority_14_final_review.md). Все шесть slices реализованы отдельными коммитами. Добавлены campaign-scoped уникальные configurations и instances, resolved/visible item views, знания персонажа и партии, hidden binding restriction и различимый аудит принудительной выдачи, перемещения и исправления.

### Приоритет 15 — read-проекции Inventory/Commerce и UI MVP-1 — завершён

**Цель.** Открыть игрокам уже реализованные backend-возможности инвентаря, дарения и партийного хранилища через campaign-scoped read API и первый законченный UI-срез «посмотреть и передать».

**Проблема.** `Inventory` и `Commerce` предоставляют команды, но почти не имеют read-проекций для клиентских экранов. Игрок не видит runtime-инвентарь, кошелёк, входящие операции и партийное хранилище, а монолитный `CampaignsView` не предоставляет навигационного якоря для новых разделов. Поэтому завершённые backend-приоритеты 9–14 недоступны в основном пользовательском потоке.

**Предлагаемые slices:**

1. Campaign-scoped read-проекция инвентаря персонажа: контейнеры, экземпляры с version и кратким resolved description, количество, экипировка, provenance и серверно вычисленная Масса.
2. Read-проекции входящих и исходящих подарков, ожидающих обменов и партийного хранилища с политикой доступа и последними аудируемыми операциями.
3. Минимальные Commerce read-зависимости экрана инвентаря: кошелёк персонажа и список поселений/магазинов кампании без реализации магазина игрока.
4. Страница кампании с отдельным URL и вкладками по [ТЗ-01](ui_trading_specs/01_campaign_page_tabs.md).
5. Инвентарь персонажа и общие UI-компоненты по [ТЗ-02](ui_trading_specs/02_character_inventory.md), включая единый формат денег, Массу, статусы операций, snackbar и обработку optimistic concurrency.
6. Подтверждаемое дарение по [ТЗ-03](ui_trading_specs/03_gift_dialog.md).
7. Партийное хранилище по [ТЗ-04](ui_trading_specs/04_party_storage_tab.md).
8. Polling ожидающих операций, campaign-scoped authorization, ru/en localization, backend integration tests и frontend Vitest/lint/build quality gate по [сквозным требованиям UI-пакета](ui_trading_specs/00_overview_and_shared.md).

**Критерии готовности:**

- пользователь открывает отдельную страницу доступной кампании и переходит к инвентарю назначенного персонажа;
- UI получает состав инвентаря, версии экземпляров, кошелёк, Массу и ожидающие операции только из серверных проекций и не пересчитывает доменные правила самостоятельно;
- подарок проходит полный цикл создания и принятия, а зарезервированный экземпляр недоступен для конфликтующей операции;
- вклад и изъятие из партийного хранилища обновляют read-модель и сохраняют различимый аудит;
- конфликт `ExpectedItemVersion` приводит к понятному сообщению и refetch без потери контекста формы;
- повтор команды с тем же operation key не создаёт дубликатов;
- данные и действия изолированы кампанией, а UI не раскрывает скрытые свойства предметов;
- ru/en, loading, empty, error и retry states покрыты сквозным Definition of Done;
- существующие campaign, inventory и commerce command flows продолжают проходить regression tests.

**Вне охвата:** магазин игрока, стол обмена и административная панель остаются UI MVP-2/MVP-3; отклонение подарка и настройка политики партийного хранилища требуют отдельных backend-команд и не имитируются клиентом.

**Статус приоритета 15:** завершён 26 июля 2026 года; выполнен [итоговый cross-review](priority_15_final_review.md). Реализованы campaign-scoped read-проекции инвентаря, ожидающих операций, партийного хранилища, кошелька и магазинов; отдельные страницы кампании и инвентаря; подтверждаемое дарение; вклад и изъятие из партийного хранилища; polling ожидающих подарков и обменов; общие UI-компоненты, ru/en и полный regression quality gate. Магазин игрока, стол обмена, настройка storage policy и административный UI остаются в явно указанном вне охвата.

### Приоритет 16 — генерация ассортимента
...
**Статус приоритета 16:** завершён 27 июля 2026 года; выполнен [итоговый cross-review](priority_16_final_review.md). Реализованы неизменяемые версии правил пополнения, ограничения по уровню, редкости, доступу, категории и бюджету, раздельные веса типов предметов, воспроизводимые запуски по seed и версии правила, предварительный просмотр, подтверждение и отклонение ведущим, идемпотентная публикация предложений и уникальных экземпляров. Редкость хранится в ревизии `ItemCatalog` и передаётся генератору отдельно от campaign scope.

### Приоритет 17 — расширенный жизненный цикл предметов
...
**Статус приоритета 17:** завершён 27 июля 2026 года; выполнен [итоговый cross-review](priority_17_final_review.md). Реализованы runtime-заряды, расходование и боеприпасы, прочность и ремонт, прикрепляемые руны, campaign-scoped server commands и trusted integration port для будущих `Gameplay`/`Encounter`. Cross-review дополнительно связал доступ к прикреплённой руне с текущим носителем и запретил её самостоятельное перемещение.

### Приоритет 18 — UI торговли MVP-2/MVP-3

**Цель.** Завершить пользовательский и административный UI поверх реализованных `Inventory`, `Commerce` и `ItemCatalog`: дать игроку магазин и атомарный обмен, а ведущему — экран управления торговлей и принудительными операциями.

**Проблема.** После Priority 15 игрок сможет просматривать и передавать предметы, но покупка, продажа и двусторонний обмен останутся доступны только через API. Ведущий также не получит интерфейс для поселений, магазинов, предложений, ценовых политик, кошельков и аудируемого force-move. Без оставшихся read-проекций существующие команды невозможно безопасно связать в полноценные экраны.

**Предлагаемые slices:**

1. Commerce read-проекции поселений и магазинов, активного и полного ассортимента, кошельков, резервов покупки и предварительной цены продажи.
2. Магазин игрока по [ТЗ-05](ui_trading_specs/05_player_shop.md): товары, резерв с таймером, оплата, отмена, продажа и список резервов.
3. Детальные read-проекции списка и карточки обмена, включая стороны, линии, статусы, версии и видимые описания предметов.
4. Стол обмена по [ТЗ-06](ui_trading_specs/06_exchange_table.md): сборка предложения, резервирование линий, подтверждение контрагентом и отмена.
5. Административные проекции всех кошельков кампании, контейнеров, полного ассортимента и поиск опубликованных ревизий `ItemCatalog`.
6. Панель торговли ведущего по [ТЗ-07](ui_trading_specs/07_commerce_admin.md): поселения, магазины, предложения, ценовая политика, корректировки кошельков и аудируемый force-move.
7. Интеграция со сквозными компонентами, polling, campaign-scoped access, optimistic concurrency, идемпотентностью, ru/en и quality gate из [общего UI-ТЗ](ui_trading_specs/00_overview_and_shared.md).

**Критерии готовности:**

- игрок видит магазины доступной кампании, активные предложения, ценовую политику и доступный остаток кошелька;
- покупка проходит полный цикл резервирования и оплаты, а истёкший или отменённый резерв освобождает деньги и товар;
- продажа использует серверную котировку, перемещает экземпляр и зачисляет точную сумму в неизменяемый денежный журнал;
- контрагент видит полный разрешённый состав обмена и может атомарно завершить или отменить его без частичного перемещения предметов;
- конфликт версии конкретного предмета обновляет проекцию и не создаёт дублирующей операции;
- ведущий управляет только торговлей своей кампании, а опасные корректировки и force-move требуют причины и сохраняют различимый аудит;
- скрытые свойства и недоступные административные данные не попадают в пользовательские ответы;
...
### Приоритет 19 — UX-подсказки заблокированных действий и прозрачность шагов UI

**Цель.** Устранить «серые кнопки без объяснения»: каждое заблокированное действие (`Далее`, `Создать`, `Пригласить` и другие CTA) сопровождается видимой причиной, шаги мастера показывают прогресс и оставшиеся требования, а перегруженные экраны разделяют выборы и справку.

**Проблема.** UI-ревью 2026-07 показало: кнопка «Далее» мастера на 10 из 11 шагов блокируется молча (связка «кнопка ↔ причина» реализована только для Ведьмы на шаге заклинаний), формы кампаний блокируют четыре кнопки без подсказок, на шагах с лимитом выбора нет счётчиков, обязательные поля не помечены, в русском UI отображаются сырые идентификаторы (`domain.cities`) и служебные заметки разработчика, а шаг «Класс» перегружен. Полное ТЗ с приоритетами и критериями приёмки: [ui_review_2026_07_disabled_states_and_hints.md](ui_review_2026_07_disabled_states_and_hints.md).

**Выполненные slices (соответствуют П1–П10 ТЗ):**

1. Общий механизм чек-листа «Чтобы продолжить: …» для каждого шага мастера по образцу `witchSpellLoadoutRequirements`, с `aria-describedby` на кнопке «Далее».
2. Подсказки и правила валидации у форм кампаний (создание, приглашение, группа, назначение персонажа) на новой странице деталей кампании.
3. Счётчики «Выбрано: N из M» на шагах с лимитом выбора (бусты, чары) по образцу шага финальных бустов.
4. Маркировка обязательных полей единым стилем во всех формах.
5. Устранение сырых идентификаторов и непереведённых строк каталога на шагах 5–7; единая стратегия локализации доменных названий.
6. Удаление служебных заметок разработчика из пользовательских текстов.
7. Разделение выборов и справки на шаге «Класс» (сгруппированные выборы + сворачиваемая справка).
8. Живая валидация Login/Register до сабмита.
9. Защита от потери прогресса мастера при перезагрузке страницы.
10. Подтверждение необратимых действий кампании (архивация, снятие роли ведущего, выход).

**Критерии готовности:**

- ни одна заблокированная CTA-кнопка не остаётся без видимого текста причины, доступного также скринридеру;
- объяснения не полагаются на hover-tooltip у disabled-кнопок;
- на шагах с лимитом выбора виден текущий прогресс;
- в пользовательском UI нет сырых идентификаторов и заметок о внутреннем устройстве;
- необратимые действия требуют явного подтверждения с описанием последствий;
- все новые строки проходят через i18n, эталонные паттерны (чек-лист Ведьмы, alert финализации, счётчик шага 8) зафиксированы как норма для новых экранов.

**Статус приоритета 19:** завершён 27 июля 2026 года; выполнен [итоговый cross-review](priority_19_final_review.md). Все десять slices реализованы отдельными коммитами. Cross-review дополнительно сохранил выборы навыков класса при восстановлении черновика и согласовал защиту последнего ведущего с доменными инвариантами сервера.
...
- Priority 11 добавляет экземпляры и миграцию; Priority 12 добавляет подарок, обмен и общее хранилище;
...
- Priority 15 добавляет пользовательский цикл открытия кампании, просмотра runtime-инвентаря, подтверждаемого подарка и работы с партийным хранилищем через реальные read-проекции;
- Priority 16 проверяет воспроизводимость генерации, соблюдение ограничений политики, однократную публикацию уникальных экземпляров и обязательный GM review;
- Priority 18 добавляет репрезентативные пользовательские покупку, продажу и обмен, а также административное управление торговлей;
- Priority 19 добавляет unit-проверки требований мастера, форм кампаний, catalog fallback, auth validation, черновика и защиты последнего ведущего;
- unit, integration и frontend tests остаются обязательными для каждого ограниченного среза.

## После приоритетов 8–19
