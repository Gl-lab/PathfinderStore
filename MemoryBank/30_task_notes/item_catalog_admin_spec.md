...
- Существующие конфигурации ревизии экран получает через `GET /api/item-catalog/revisions` (проекция Commerce). Проекция расширена: `ItemConfigurationAdministrationDto` отдаёт `campaignId` и постоянные улучшения (endpoint GM-only, скрытые улучшения — секреты самого ведущего), поэтому конфигурации с одинаковой тройкой, но разными улучшениями различимы в чипах и опциях, а клиентская проверка дубликата точна и блокирует создание (disabled + подсказка, по исходному ТЗ). Legacy-конфигурации (`campaignId == null`) из блокировки исключены — их ключ не совпадает с ключом новой кампанийной конфигурации.
